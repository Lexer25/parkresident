    <?php defined('SYSPATH') or die('No direct script access.');
 
    class Task_CheckUhfExit31 extends Minion_Task {
		/**
		
		Тест для имитации обработки быстро пришедших идентификаторов.
		C:\xampp\php\php.exe c:\xampp\htdocs\parkresident\modules\minion\minion --task=CheckUhfExit31
		
		*/
		
		    protected $_options = array(
        // param name => default value
        'cam'   => 6,//'это камера выезда
        'grz'   => 'A005BB177',
     //   'key'   => '50500505',
        'key'   => '8696164',
        'ip'   => '172.16.101.105',//шкаф выезда
        'ch'   => 0,
  
		);
	
        
        protected function _execute(array $params)
        {
        
		$data=array (
			'camera' => Arr::get($params, 'cam'),
			'channel' => 3,
			'count' => 16,
			'dateTime' => '20250429T141918Z',
			'description' =>'---',
			'direction' => 0,
			'groupId' => -1,
			'id' => 624101,
			//'image' =>  '&nbsp;&hellip;',
			'inList' => 0,
			'passed' => 1,
			//'plate' => 'C023CA797',
			'plate' => Arr::get($params, 'grz'),
			'quality' =>  '555555555000',
			'stayTimeMinutes' => 0,
			'type' => 0,
			'weight' => 0,
			'test' => 1
			);
			//echo Debug::vars('98', $data);exit;
			
		$data_0=array (
				'key' => strtoupper(dechex(Arr::get($params, 'key'))),
				'test' => 1,
				'ip'=>Arr::get($params, 'ip'),
				'ch'=>Arr::get($params, 'ch'),
				);

		//Model::factory('Emul')->sendRequestPostJson(Array('plate'=>$data), 'dashboard/exec', 'body');
		Model::factory('Emul')->sendRequestPostJson($data_0, 'dashboard/sendMPT', 'post');
		
		
        }
    }