...
class Controller_Grz extends Controller_Template { // класс описывает въезды и вызды (ворота) для парковочных площадок
	...
	//echo Debug::vars('51 car_in_parking', $_POST); exit;
		...
		$query->rule('grz', 'not_empty')
			//->rule('grz', 'regex', array(':value', '/^[A-Z\d]{10}+$/')) // https://regex101.com/ допустимы знаки только ГРЗ
			;
		if($query->check())
		{
...
*/
	...
	{
		//echo Debug::vars('99 car_in_parking', $_POST); exit;
...
Имитируется работа системы распознавания ГРЗ
...
*/	
	...
	public function action_test_car_parking()
	{
		...
		if($post->check())
		{
			...
			try{
				//эмулирую подачу пакета, аналогичного CVS, на контроллер cvstest/exec
