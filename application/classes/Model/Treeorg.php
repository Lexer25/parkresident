...
public function tplMenu($category){ //Шаблон для вывода меню в виде дерева
	...
	if(is_array(Arr::get($category, 'childs')))
	{
		...
		if(Arr::get($category, 'busy') != $this->id_parking) $menu = '<li>'.Form::checkbox('id_org_for_add_garage['.Arr::get($category, 'id').']', Arr::get($category, 'id'), TRUE, array("disabled"=>"disabled")).'(id='.Arr::get($category, 'id').') '.Arr::get($category, 'title');// организация занята, нельзя управлять
		if(Arr::get($category, 'busy') == $this->id_parking) $menu = '<li>'.Form::checkbox('id_org_for_add_garage['.Arr::get($category, 'id').']', Arr::get($category, 'id'), TRUE).'(id='.Arr::get($category, 'id').') '.Arr::get($category, 'title'); // должна стоять галочка, разрешено снимать.
		if(is_null(Arr::get($category, 'busy'))) $menu = '<li>'.Form::checkbox('id_org_for_add_garage['.Arr::get($category, 'id').']', Arr::get($category, 'id'), FALSE).'(id='.Arr::get($category, 'id').') '.Arr::get($category, 'title'); // свободна, галочка снята, можно выбирать.
	}
	...
	**/
	...
	public function make_tree($org_list, $id_garage)// дерево с расставленными метками: кого можно выбрать, кого нельзя. $org_list  - лист организаций(для построения дерева), $org_busy - список организаций, уже занятых где-то, $garage_info - информация о гараже
	{
		//$id_parking
